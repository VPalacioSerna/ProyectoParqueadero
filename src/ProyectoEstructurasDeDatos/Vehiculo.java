...
public class Vehiculo {
	...
	protected int tiempoInicial = Tiempo.hora();
	...
	public Vehiculo(String color, String marca, String placa, String numeroEstacionamiento) {
		super();
		this.color = color;
		this.marca = marca;
		this.placa = placa;
	...
	/*
	 * Significado simbolico de el metodo matches: ^ = inicio $ = fin {3} = 3 veces
	 * repetidas \\d = cualquier digito del 0 al 9
	 */
	 ...
	 * bueno añadir algun tipo de factor que diferencie los carros de las motos,
	 * como para que no parezca que tienen exactamente los mismos atributos, porque
	 * entonces no tendria mucho sentido el cambio que hice
	 */
	public static void mostrarVehiculos() {
		...
		for (int i = 0; i < Carro.espaciosPisoCarro.length; i++) {
			if (Carro.espaciosPisoCarro[i] != false) {
				...
				System.out
						.print("Estacionado en el puesto: " + Carro.espaciosCarro[i].getNumeroEstacionamiento() + "\n");
				System.out.println();
			} else {
				continue;
			}
		...
		for (int i = 0; i < Carro.espaciosPisoCarro.length; i++) {
			if (Carro.espaciosPisoCarro[i] != false) {
				if (Carro.espaciosCarro[i].getPlaca().equals(placa)) {
					...
					System.out.print("Placa: " + Carro.espaciosCarro[i].getPlaca() + "\n");
					System.out.print(
							"Estacionado en el puesto: " + Carro.espaciosCarro[i].getNumeroEstacionamiento() + "\n");
					System.out.println();
				}
			} else {
				continue;
		...
		for (int i = 0; i < Moto.espaciosPisoMoto.length; i++) {
			if (Moto.espaciosPisoMoto[i] != false) {
				if (Moto.espaciosMoto[i].getPlaca().equals(placa)) {
					...
					System.out.print("Placa: " + Moto.espaciosMoto[i].getPlaca() + "\n");
					System.out.print(
							"Estacionado en el puesto: " + Moto.espaciosMoto[i].getNumeroEstacionamiento() + "\n");
					System.out.println();
				}
			} else {
				continue;
